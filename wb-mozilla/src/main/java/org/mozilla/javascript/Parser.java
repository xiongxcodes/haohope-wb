...
public class Parser {
	...
	private Node statementHelper(Node statementLabel) throws IOException,
		...
		switch (tt) {
			...
			case 72 :
				...
				break;
			case 39 :
				arg75 = this.ts.getLineno();
				String arg74 = this.ts.getString();
				...
				if (pn.getType() == 129) {
					...
					this.consumeToken();
					this.decompiler.addName(arg74);
					this.decompiler.addEOL(102);
					if (this.labelSet == null) {
						this.labelSet = new HashMap();
					} else if (this.labelSet.containsKey(arg74)) {
					...
					this.labelSet.put(arg74, statementLabel);

					try {
						pn = this.statementHelper(statementLabel);
					} finally {
						this.labelSet.remove(arg74);
					}
				...
				pn = this.nf.createExprStatement(pn, arg75);
				break;
			case 50 :
				this.consumeToken();
			...
			case 80 :
				...
				if (arg72 == 123) {
					while (this.matchToken(123)) {
						...
						String arg76 = this.ts.getString();
						this.decompiler.addName(arg76);
						Node catchCond = null;
						...
						this.nf.addChildToBack(
								expr,
								this.nf.createCatch(arg76, catchCond,
			...
			case 84 :
				...
				arg68 = this.nf.createScopeNode(128, this.ts.getLineno());
				this.pushScope(arg68);

				try {
					this.statements(arg68);
			...
			case 115 :
				...
				expr = this.expr(false);
				pn = this.nf.createDefaultNamespace(expr, arg69);
				break;
			case 116 :
				this.consumeToken();
				this.decompiler.addToken(116);
				arg68 = this.enterLoop(statementLabel, true);
				...
				try {
					...
					expr = this.statement();
					this.decompiler.addEOL(85);
					pn = this.nf.createWhile(arg68, nsLine, expr);
				} finally {
					this.exitLoop(true);
				}
			...
			case 117 :
				...
				try {
					...
					this.decompiler.addToken(116);
					expr = this.condition();
					pn = this.nf.createDoWhile(arg68, nsLine, expr);
				} finally {
					this.exitLoop(true);
				}
	...
	private Node arrayComprehension(String arrayName, Node expr)
		...
		if (this.nextToken() != 118) {
			...
			try {
				cond = this.nf.createForIn(152, loop, init, iterator, body,
						isForEach);
			} finally {
				this.exitLoop(false);
			}
	...
	private Node primaryExpr() throws IOException, Parser.ParserException {
		...
		switch (tt) {
			...
			case 99 :
				...
				condComment = this.ts.regExpFlags;
				this.ts.regExpFlags = null;
				keepComment = this.ts.getString();
				this.decompiler.addRegexp(keepComment, condComment);
				arg21 = this.currentScriptOrFn.addRegexp(keepComment,
						condComment);
				return this.nf.createRegExp(arg21);
			case 39 :
				condComment = this.ts.getString();
				if ((ttFlagged & 131072) != 0 && this.peekToken() == 102) {
					return this.nf.createLabel(this.ts.getLineno());
				}

				this.decompiler.addName(condComment);
				if (this.compilerEnv.isXmlAvailable()) {
					pn = this.propertyName((Node) null, condComment, 0);
				} else {
					pn = this.nf.createName(condComment);
				}
			...
			case 41 :
				condComment = this.ts.getString();
				this.decompiler.addString(condComment);
				return this.nf.createString(condComment);
			case 42 :
			case 43 :
			case 44 :
			case 45 :
					...
					while (true) {
						...
						if (tt == 88) {
							...
							if (tt == 83) {
								this.consumeToken();
								this.decompiler.addToken(83);
								arg21 = arg16.size() + (arg22 ? 1 : 0);
								return this.nf.createArrayLiteral(arg16, arg19,
										arg21);
							}

							if (arg19 == 0 && arg16.size() == 1 && tt == 118) {
							...
							arg16.add(this.assignExpr(false));
							arg22 = false;
						}
					}
				}
			case 84 :
							...
							case 41 :
								...
								if (tt == 39) {
									if (s.equals("get")
										...
										if (!this.getterSetterProperty(arg16,
												index, true)) {
											break label245;
										}
										break;
								...
								index = ScriptRuntime.getIndexObject(s);
								this.plainProperty(arg16, index);
								break;
							case 40 :
								this.consumeToken();
				...
				this.decompiler.addToken(85);
				return this.nf.createObjectLiteral(arg16);
			case 86 :
				this.decompiler.addToken(86);
				pn = this.expr(false);
			...
			case 160 :
				condComment = this.ts.getString();
				this.decompiler.addJScriptConditionalComment(condComment);
				return this.nf.createString(condComment);
			case 161 :
				keepComment = this.ts.getString();
				this.decompiler.addPreservedComment(keepComment);
				return this.nf.createString(keepComment);
			default :
				this.reportError("msg.syntax");
		}
