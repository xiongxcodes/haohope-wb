 ...
 *     grid.filters
 */
Ext.define('Ext.ux.grid.ToolbarFiltersFeature', {
    extend: 'Ext.grid.feature.Feature',
    alias: 'feature.tbarfilters',
    uses: [
    ...
    init: function(grid) {
        ...
        if(grid.columnManager.getColumns().length > 0)
        {
	        me.createFilters();
            me.initColumns();
        }else{
    ...
    onSelchange:function(grid,selected){
	    var btns = Ext.ComponentQuery.query('button[_selrecord_=true]');
        for(var i=0;i<btns.length;i++){
	        btns[i].setDisabled(selected.length<=0);
        }
    },
    onRender:function(grid){
	    if(grid._buttons_){
                var tbar = grid.down('toolbar');
                grid._buttons_.forEach(function(e) {
                  tbar.add(Wb.apply(e, {
                      handler:function(btn){
                        if(btn._handler_) {eval(' var _handler_ = function(){'+btn._handler_+'}') ; _handler_();}
                      },
                      listeners:{
	                    afterrender:function(btn){
		                   if(btn._afterrender_) {eval(' var _afterrender_ = function(){'+btn._afterrender_+'}') ; _afterrender_();}
	                    }
                      }
                  }));
                });
    ...
    onCheckChange2: function (item, value) {
    ...
    onBeforeCheck2: function (item, value) {
    ...
    initColumns: function () {
        var grid = this.grid,store = grid.getStore(),columns = grid.columnManager.getColumns(),column,id;

        // We start with filters defined on any columns.
        //c4w create menu on toolbar
        var me = this;
        me.initializeFilters(me.filters.items);
        var its = [];
        for(var i=0;i<columns.length;i++)
        {
	       column = columns[i];
	       column.filter = this.getFilterByDataIndex(column.dataIndex);
           if(!column.filter){
	          continue;
           }
           column.filter.menuitemid = Ext.id();
           its.push({
	                id:column.filter.menuitemid,
                    text: column.text,
                    checked: false,
                    ...
                    listeners: {
                       scope: me,
                       checkchange: me.onCheckChange2,
                       beforecheckchange: me.onBeforeCheck2
                    }
           });
        }
        var items = [{
              ...
              hidden:its.length==0,
              menu: its,
              handler: me.onClearFilters,
              scope: me
	       },{
		      xtype:'button',
		      tooltip: '清理查询',
              iconCls: 'filter_delete_icon',
              hidden:its.length==0,
              handler: me.onClearFilters,
              scope: me
	       }];
        ...
        try{
          ...
          if(tbar.items.length > 0 && items.length > 0){
	         tbar.add('-');
          }
          tbar.add(items);
        }catch(e){
	      tbar = Ext.create('Ext.toolbar.Toolbar', grid.tbar);
          if(tbar.items.length > 0 && items.length > 0){
	         tbar.add('-');
          }
          tbar.add(items);
    ...
    onStateChange : function (event, filter) {
        if (event !== 'serialize') {
            ...
            try{
	           Ext.getCmp(filter.menuitemid).setChecked(filter.active, false);
            }catch(e){}
    ...
    buildQuery : function (filters) {
        ...
        if (!this.encode){
            for (i = 0; i < len; i++) {
                ...
                f = filters[i];
                param = {};
                if(!f.field || ''===f.field) continue;
                for(key in f.data) {
                    if(!f.data[key] || ''===f.data[key])right=false;
                }
                if(false === right) continue;
                for(key in f.data) {
                    //p[[dataPrefix, '[', key, ']'].join('')] = f.data[key];
                    param[key] = f.data[key];
                }
                param['field'] = f.field;
                params.push(param);
                /**
