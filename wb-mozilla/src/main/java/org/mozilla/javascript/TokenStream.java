...
class TokenStream {
		...
		label454 : while (true) {
			...
			if (!isJSSpace(c)) {
					...
					while (true) {
						while (true) {
							...
							while (isUnicodeEscapeStart) {
								...
								for (arg16 = 0; arg16 != 4; ++arg16) {
									c = this.getChar();
									arg15 = Kit.xDigitToInt(c, arg15);
									if (arg15 < 0) {
										break;
									}
								}
							...
							if (c != 92) {
								if (c == -1
									...
									this.ungetChar(c);
									sb = this.getStringFromBuffer();
									if (!arg11) {
										arg16 = stringToKeyword(sb);
										if (arg16 != 0) {
											if ((arg16 == 152 || arg16 == 72)
													&& this.parser.compilerEnv
															.getLanguageVersion() < 170) {
												this.string = arg16 == 152
														? "let"
														: "yield";
												arg16 = 39;
											}
				...
				if (isDigit(c) || c == 46 && isDigit(this.peekChar())) {
					...
					this.ungetChar(c);
					s1 = this.getStringFromBuffer();
					double arg17;
					if (arg12 == 10 && !arg14) {
						try {
							arg17 = Double.valueOf(s1).doubleValue();
						} catch (NumberFormatException arg9) {
							this.parser.addError("msg.caught.nfe");
							return -1;
						}
					} else {
						arg17 = ScriptRuntime.stringToNumber(s1, 0, arg12);
					}
				...
				for (c = this.getChar(); c != lookForSlash; c = this.getChar()) {
					if (c == 10 || c == -1) {
						this.ungetChar(c);
				...
				sb = this.getStringFromBuffer();
				this.string = (String) this.allStrings.intern(sb);
				return 41;
			}
		}
	}
	...
	void readRegExp(int startToken) throws IOException {
		...
		int c;
		for (boolean inClass = false; (c = this.getChar()) != 47 || inClass; this
				.addToString(c)) {
			if (c == 10 || c == -1) {
				this.ungetChar(c);
